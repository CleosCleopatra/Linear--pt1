...
include("patameter_values_sensitivity_e.jl")

function build_model(parameter_value_sensitivity::String, ti)
  # The diet problem
...
for j in 1:length(t)
    m, a, B, D, M = build_model("patameter_values_sensitivity_e.jl", t[j])
    #print(m) # prints the model instance

    #set_optimizer(m, model)
    optimize!(m)

    println("tax_rate: ", t[j])
